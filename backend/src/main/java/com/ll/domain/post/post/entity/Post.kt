...
class Post : BaseTime {
    ...
    fun checkActorCanRead(actor: Member?) {
        Optional.of(getCheckActorCanReadRs(actor))
            .filter { rsData -> rsData.isFail }
            .ifPresent { rsData ->
                throw ServiceException(rsData.resultCode, rsData.msg);
            }
    ...
    private fun processGenFile(
        ...
        if (Ut.str.isNotBlank(metadataStrFromFileName)) {
            metadataStr = if (Ut.str.isBlank(metadataStr)) {
        ...
        val fileName = if (isModify) {
            Ut.file.withNewExt(oldPostGenFile!!.fileName, fileExt)
        } else  {
            UUID.randomUUID()
                .toString() + "." + fileExt
        }
        ...
        val actualFileNo = if(fileNo == 0) {
            getNextGenFileNo(typeCode)
        } else {
            fileNo
        }
        ...
        val genFile = if (isModify) {
            ...
            PostGenFile(this, typeCode, actualFileNo)
        }
